...
public partial class BlockingAnalyzerWindow : Window
{
    ...
    private async void LocksButton_Click(object sender,RoutedEventArgs e){if(Selected() is not{}x)return;try{AnalyzerStatus.Text=En?$"Querying locks for SPID {x.SessionId}...":$"Consultando locks SPID {x.SessionId}...";DetailText.Text=await _service.GetBlockingLocksAsync(x.SessionId);AnalyzerStatus.Text=En?$"Locks for SPID {x.SessionId} completed.":$"Locks SPID {x.SessionId} finalizado.";}catch(Exception ex){AnalyzerStatus.Text="ERROR locks: "+ex.Message;}}
    private void EvidenceButton_Click(object sender,RoutedEventArgs e){if(Selected() is not{}x)return;var text=SqlHealthService.BuildBlockingEvidence(x,_items)+"\n\n"+IncidentDiagnosisEngine.DiagnoseBlocking(x,_items);DetailText.Text=text;Clipboard.SetText(text);AnalyzerStatus.Text=En?$"Evidence Snapshot + diagnosis for SPID {x.SessionId} copied.":$"Evidence Snapshot + diagnóstico SPID {x.SessionId} copiado.";}
