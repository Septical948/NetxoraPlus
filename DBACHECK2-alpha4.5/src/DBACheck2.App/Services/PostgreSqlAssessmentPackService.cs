...
public sealed class PostgreSqlAssessmentPackService
{
    ...
    public async Task<List<AssessmentCheck>> RunFullAsync()
    {
        ...
        x.Add(await Q(c,"PG.MAINT.VACUUM",AssessmentCategory.Maintenance,"Vacuum / Analyze Recency",@"SELECT CASE WHEN COUNT(*) FILTER (WHERE COALESCE(last_autovacuum,last_vacuum) IS NULL)>0 THEN 'WARNING' ELSE 'OK' END,COUNT(*) FILTER (WHERE COALESCE(last_autovacuum,last_vacuum) IS NULL)||' table(s) without vacuum timestamp',COALESCE(string_agg(schemaname||'.'||relname||' dead='||n_dead_tup, '; '),'') FROM pg_stat_user_tables;"));
