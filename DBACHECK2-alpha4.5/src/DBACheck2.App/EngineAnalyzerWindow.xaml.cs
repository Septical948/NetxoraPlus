...
public partial class EngineAnalyzerWindow:Window
{
    ...
    async Task LoadAsync(){try{ContextText.Text=$"{provider.DisplayName} | consultando...";var all=await provider.QuickCheckAsync();var rows=Filter(all,module).ToList();GridData.ItemsSource=rows;ContextText.Text=$"{provider.DisplayName} | {rows.Count} check(s) | {LocalizationService.T("Common.ReadOnly")}";}catch(Exception ex){ContextText.Text="ERROR: "+ex.Message;}}
