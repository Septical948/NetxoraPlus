...
public partial class AssistantProfilesWindow:Window
{
    ...
    public AssistantProfilesWindow(string currentHost)
    {
        ...
        EngineBox.ItemsSource=Enum.GetValues<DatabaseEngine>(); EngineBox.SelectedItem=DatabaseEngine.SqlServer;
    ...
    private void UpdateEngineOptions(){var e=(DatabaseEngine)(EngineBox.SelectedItem??DatabaseEngine.SqlServer);SshPanel.Visibility=e is DatabaseEngine.MySqlMariaDb or DatabaseEngine.PostgreSql?Visibility.Visible:Visibility.Collapsed;if(e==DatabaseEngine.MySqlMariaDb&&string.IsNullOrWhiteSpace(PortBox.Text))PortBox.Text="3306";else if(e==DatabaseEngine.PostgreSql&&string.IsNullOrWhiteSpace(PortBox.Text))PortBox.Text="5432";else if(e==DatabaseEngine.Oracle&&string.IsNullOrWhiteSpace(PortBox.Text))PortBox.Text="1521";}
    ...
    private void ClearEditor(){ProfilesBox.SelectedItem=null;ProfileName.Clear();HostBox.Text="localhost";PortBox.Clear();DatabaseBox.Clear();UsernameBox.Clear();PasswordBox.Clear();RememberPasswordBox.IsChecked=false;UseSshBox.IsChecked=false;SshHostBox.Clear();SshPortBox.Text="22";SshUserBox.Clear();SshKeyPathBox.Clear();SshPassphraseBox.Clear();}
